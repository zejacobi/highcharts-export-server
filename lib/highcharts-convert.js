...
(function() {
  ...
  render = function(params, exitCallback) {
    ...
    createChart = function(constr, input, themeOptionsArg, globalOptionsArg, dataOptionsArg, customCodeArg, outputType, callback) {
      ...
      } else if (JSON.parse(input).globalOptions != undefined) {
        globalOptions = JSON.parse(input).globalOptions;
        loadScript('globalOptions', JSON.stringify(JSON.parse(input).globalOptions));
      }
      ...
      options.chart.width = (options.exporting && options.exporting.sourceWidth) || options.chart.width || options.width || 600;
      options.chart.height = (options.exporting && options.exporting.sourceHeight) || options.chart.height || options.height || 400;

      if (globalOptions) {
        if (globalOptions.timezoneOffset) {
          globalOptions.useUTC = true;
          Highcharts.setOptions({global: globalOptions});
        } else {
      ...
      if (dataOptions) {
		...
		console.log(JSON.stringify(options.chart.style));
    ...
    if (params.length < 1) {
      ...
      page.open('about:blank', function() {
        ...
        if (input.substring(0, 4).toLowerCase() === '<svg' || input.substring(0, 5).toLowerCase() === '<?xml' ||
          ...
          var htmlParts = svg.html.split('<defs>');
          // svg.html = htmlParts[0] + "<defs><style type=\"text/css\">\n@font-face {\n" +
          //   "  font-family: 'Roboto Condensed';\n" +
          //   "  font-style: normal;\n" +
          //   "  font-weight: 400;\n" +
          //   "  src: url(https://fonts.gstatic.com/s/robotocondensed/v14/Zd2E9abXLFGSr9G3YK2MsDAdhzWOYhqHvOZMRGaEyPo.woff2) format('woff2');\n" +
          //   "  unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2212, U+2215;\n" +
          //   "}</style>" + htmlParts[1];
          svg.html = htmlParts[0] + "<defs><style type=\"text/css\"> @import url('https://fonts.googleapis.com/css?family=Roboto+Condensed');</style>" + htmlParts[1];
          renderSVG(svg);
        }
      });
    }
